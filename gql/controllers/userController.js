...
const userRegisterController = async (args, context) => {
  ...
  try {
    ...
    const newUser = { mobileNumber, passWord };
    userDB.setUsers([...userDB.users, newUser]);
    return await fsPromises
      .writeFile(
        path.join(__dirname, '../../localDB', 'users.json'),
...
const userLoginController = async (args, context) => {
  ...
  const passwordCheckResult = passWord === user.newPassword;
  if (!passwordCheckResult) throw new CustomException(409, 'Wrong password');
  try {
    var token = jwt.sign({ mobileNumber }, 'shhhhh', { expiresIn: 60 });
    var refreshToken = jwt.sign({ mobileNumber }, 'shhhhh', {
      expiresIn: 60 * 60,
    });
    user.refreshToken = refreshToken;

    const filteredUsers = userDB.users.filter(
      (user) => user.mobileNumber != mobileNumber
    );
    return await fsPromises
      .writeFile(
        path.join(__dirname, '../../localDB', 'users.json'),
        JSON.stringify([...filteredUsers, user])
      )
      .then(() => {
...
const userRefreshController = async (args, context) => {
  try {
    ...
    const newToken = jwt.sign({ mobileNumber: user.mobileNumber }, 'shhhhh', {
      expiresIn: 60,
    });
